...
public class SokoAMDP1 implements DomainGenerator{
	...
	public static State projectToAMDPState(State s, Domain aDomain){
		...
		for(ObjectInstance d : doors){
			ObjectInstance ad = new MutableObjectInstance(aDomain.getObjectClass(Sokoban2Domain.CLASSDOOR), d.getName());
			as.addObject(ad);
		}
		...
		for(ObjectInstance b : blocks){
			ObjectInstance ab = new MutableObjectInstance(aDomain.getObjectClass(Sokoban2Domain.CLASSBLOCK), b.getName());

 ...
 *
 */
public class VisualExplorerRecorder extends JFrame{
	...
	State											baseState;
	State											curState;
	EpisodeAnalysis 								currentEpisode;
	List<EpisodeAnalysis>							recordedEpisodes;
	Visualizer 										painter;
	TextArea										propViewer;
	 ...
	 */
	public VisualExplorerRecorder(Domain domain, Visualizer painter, State baseState){
	 ...
	 */
	public VisualExplorerRecorder(Domain domain, Visualizer painter, State baseState, int w, int h){
		this.init(domain, painter, baseState, w, h);
	}

	public VisualExplorerRecorder(Domain domain, Visualizer painter, State baseState, int w, int h, RewardFunction rf){
		this.init(domain, painter, baseState, w, h);
	...
	public List<EpisodeAnalysis> initGUIAndRecord(List<EpisodeAnalysis> recordedEpisodes) {
		this.currentEpisode = new EpisodeAnalysis(this.baseState);
		this.recordedEpisodes = recordedEpisodes;
		this.isRecording = true;
		SpecialExplorerAction action = new SpecialExplorerAction() {
			@Override
		...
		this.addSpecialAction("r", action);
		...
		if(mappedAction != null){
			...
			if(action == null){
				...
				if (this.rewardFunction != null) {
					reward = this.rewardFunction.reward(this.curState, null, null);
				}
				this.currentEpisode.recordTransitionTo(this.curState, groundedAction, reward);
				numSteps++;
			}
			...
			if(sea instanceof StateResetSpecialAction){
				...
				this.recordedEpisodes.add(this.currentEpisode);
				this.currentEpisode = new EpisodeAnalysis(this.baseState);
				numSteps = 0;
